...
- **2026-06-05: full case facts emphasized to the agent.** All 3 prompts (live + docs) now spell out THE DEBT as the exact amount, WHAT IT WAS FOR (debt_reason), WHEN IT WAS INCURRED (debt_since), the creditor, and SETTLEMENT AUTHORITY (the only settlement_options the collector may accept). Goal description also reinforces acceptable outcomes (full payment / plan / approved option). Data path was already correct (mappers.ts maps reason/since/settlement into customFields → leadInfo); this makes the agent actively use them.
